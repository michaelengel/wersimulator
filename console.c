...
{
  static char input[80], copy[80], regname[80], filename[PATH_MAX];
  char *strptr;
  tt_u16 memadr, start, end,addr;
  long n;
  int i, r;
  int regon = 0;
  ...
  for(;;) {
    ...
    case 'h' : case '?' :
      ...
      printf("   b file [adr]    : load plain binary <file> at address <adr>\n");
    ...
    case 'M' :
      if (more_params(&strptr)) {
        strcpy(filename,readstr(&strptr));
        if (filename[0]=='W') {
          wersi = 1; mirage = 0;
        } else if (filename[0]=='M') {
          mirage = 1; wersi = 0;
        } else {
	  printf("Unknown machine %c.\n", filename[0]);
        }
      } else
	printf("Current mode: %s.\n", (wersi==1?"Wersi":(mirage==1?"Mirage":"NONE")));
      break;
    case 'b' :
      if (more_params(&strptr)) {
        strcpy(filename,readstr(&strptr));
	...
	n = end = memadr;
      ...
      while (addr <= (long)end) 
      {
	printf("%04hX: ", (unsigned int)addr);                     // print addr

	// print data in hex format
	for (i = 1; i <= 16; i++,addr++)
	{
	  if( (addr>=n) && (addr<=end) ){
	    printf("%02X ", get_memb(addr)); 
	  } else {
	    printf("   ");// blank space
	  }
	}
    ...
    case 'R' :
      if (more_params(&strptr)){
	strcpy(regname,readstr(&strptr));
	if (more_params(&strptr)){
	  m6809_setreg(regname,readhex(&strptr));
	}
      }
      break;
